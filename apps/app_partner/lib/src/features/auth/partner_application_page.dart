...
class _PartnerApplicationPageState extends State<PartnerApplicationPage> {
  ...
  Future<void> _pickFile(bool isBizReg) async {
    ...
    if (file != null) {
      setState(() {
        if (isBizReg) _bizRegFile = file;
        else _bankbookFile = file;
      });
    }
  }
  ...
  Future<void> _submit() async {
    ...
    if (_bizRegFile == null || _bankbookFile == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('필수 서류를 모두 업로드해 주세요.')));
      return;
    }
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildTextField('introduction', '소개글', '사장님과 매장을 소개해 주세요.', maxLines: 3),
                    _buildTextField('address', '주소', '파티가 열릴 매장 주소'),
                    _buildTextField('contact_phone', '연락처', '010-0000-0000'),
                    _buildTextField('contact_email', '이메일', 'partner@example.com'),
                    ...
                    _buildFilePicker('사업자등록증', _bizRegFile, () => _pickFile(true)),
                    const SizedBox(height: 12),
                    _buildFilePicker('통장 사본', _bankbookFile, () => _pickFile(false)),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        child: const Text('입점 신청하기', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                      ),
                    ),
                  ],
                ),
  ...
  Widget _buildFilePicker(String label, XFile? file, VoidCallback onTap) {
    return ListTile(
      ...
      subtitle: Text(file != null ? file.name : '파일을 선택해 주세요.', style: TextStyle(color: file != null ? Colors.blue : Colors.grey)),
