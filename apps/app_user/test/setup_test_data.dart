...
void main() async {
  ...
  try {
    ...
    // But auth.users is protected. We can insert into user_profiles directly
    // if we disable triggers? No, FK constraint.
    // Let's use a known ID from seed if available, or try to insert into
    // auth.users (superuser can).
    ...
    await connection.execute(
      "INSERT INTO auth.users (id, email) VALUES ('$userUuid', "
      "'test@example.com') ON CONFLICT (id) DO NOTHING",
    );
    // Trigger on auth.users will create user_profiles automatically.

    // 2. Create Dummy Party
    await connection.execute(
      "INSERT INTO public.partners (id, name) VALUES ('$partnerUuid', "
