...
void main() {
  String generateUuid() {
    ...
    final parts = List.generate(5, (i) {
      if (i == 0) return random.nextInt(0xFFFFFFFF).toRadixString(16).padLeft(8, '0');
      if (i == 1 || i == 2 || i == 3) return random.nextInt(0xFFFF).toRadixString(16).padLeft(4, '0');
  ...
  test('E2E Pipeline Integration: DB -> Queues -> Dispatcher', () async {
    ...
    try {
      ...
      await connection.execute("INSERT INTO public.partners (id, name) VALUES ('$partnerId', 'E2E Partner') ON CONFLICT DO NOTHING");
      ...
      final qNotif = await connection.execute("SELECT * FROM pgmq.read('q_notifications', 30, 10)");
      final qVect = await connection.execute("SELECT * FROM pgmq.read('q_vectors', 30, 10)");
      
      print('q_notifications message count: ${qNotif.length}');
      print('q_vectors message count: ${qVect.length}');
      
      expect(qNotif.length, greaterThan(0), reason: "Dispatcher should fan-out to q_notifications");
      expect(qVect.length, greaterThan(0), reason: "Dispatcher should fan-out to q_vectors");

      // 5. Verify payload content
      final firstMsg = qVect.first;
      // Index 0: msg_id, Index 1: read_ct, Index 2: enqueued_at, Index 3: vt, Index 4: message
      final message = firstMsg[4] as Map<String, dynamic>; 
      expect(message['event_type'], equals('party_created'));
      expect(message['record']['id'], equals(partyId));
