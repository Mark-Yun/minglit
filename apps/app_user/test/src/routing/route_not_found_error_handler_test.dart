// Regression tests for #1699 and #1700:
...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      // Navigate to an unknown path — previously caused GoException crash
      final router = GoRouter.of(tester.element(find.text('home')));
      router.go('/minglit://dev/switch/unknown-path');
      await tester.pumpAndSettle();

      // Fix #1699: should NOT crash; error page appears instead
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      // Fix #1700: user sees friendly message, not raw exception text
      expect(find.text('페이지를 찾을 수 없습니다.'), findsOneWidget);
      expect(
