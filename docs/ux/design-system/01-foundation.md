...
| 토큰명 | Hex | 용도 | 라인 |
| :--- | :--- | :--- | :--- |
| `background` | `#FFFFFF` | 카드/컴포넌트 배경색 (흰색 팝업 효과) | :7 |
...
### 라이트 모드 배경 계층 구조 (토스 패턴)

| 레이어 | 토큰 | Hex | 설명 |
| :--- | :--- | :--- | :--- |
| Scaffold (최하단) | `surface` | `#F9FAFB` | 연한 회색 — 화면 기본 배경 |
| 카드 / 컴포넌트 | `background` | `#FFFFFF` | 흰색 — scaffold 위에서 팝업 효과 |

> 다크 모드는 반대: scaffold=`#0F0F0F`(가장 어두움) → 카드=`#212121`(조금 밝음). 변경 없음.
...
| 토큰명 | 값 (px) | 용도 |
| :--- | :--- | :--- |
| `screenEdge` | 16 | 화면 좌우 패딩 (Material 16dp 표준) |
| `cardGap` | 12 | 카드 간 간격 |
