...
| 2-1 | P0 | 14일 경과 시 PENDING → READY 전환 | • nightly cron 03:00 KST<br>• 14일 경과 PENDING 조회<br>• 체크섬 검증 통과 시 READY<br>• 검증 실패 시 HOLD + 운영 검토 | FR-4, FR-5 | NFR-2, NFR-4 |
...
- **FR-5**: 체크섬 검증 실패 시 HOLD 전환 + 운영 검토. PENDING→FAILED 전환은 상태 머신에서 허용되지 않음. 항목 무결성이 확인되기 전에 지급 불가.
...
- **NFR-5**: 정산 레코드 중복 생성: 0건 (동일 이벤트 기준, monthly audit 기준). 멱등성 위반 즉시 알림.
...
| 2-1 | 파트너 계좌 미등록 상태에서 READY 전환 시도 | 체크섬 실패 → HOLD + 운영 검토 (PENDING→FAILED 전환 불가) |
