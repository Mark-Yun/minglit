...
|-----|----------|----------|---------|----|----|
| 1-1 | P0 | 결제 승인 시 정산 레코드 자동 생성 | • 유저 결제 완료 → PortOne 확인<br>• `settlement_items` PENDING 생성<br>• 율 스냅샷 (수수료율·VAT) 저장<br>• 이중 생성 방지 (멱등성) | FR-1, FR-2, FR-3 | NFR-1, NFR-5 |
| 1-2 | P0 | 이중 결제 확인 경합 방지 | • 앱 확인 + 웹훅 동시 도달<br>• 첫 번째 처리만 정산 생성<br>• 두 번째는 중복 무시 | FR-2 | NFR-5 |
...
- **FR-1**: 결제 승인 이벤트 발생 시 정산 레코드를 자동으로 PENDING 상태로 생성. 수동 개입 없음.
- **FR-2**: 동일 결제 건에 대해 정산 레코드는 1건만 생성 (멱등성). 앱 확인 + 웹훅 중 첫 번째 처리만 유효.
...
| CUJ ID | 케이스 | 기대 동작 |
|--------|--------|----------|
| 1-1 | 결제 승인 중 DB 연결 오류 | 롤백 → 웹훅 재시도에서 처리 (멱등성 보장) |
| 1-2 | 앱 확인 + 웹훅 동시 도달 | 첫 번째 처리만 정산 생성. 두 번째는 중복 무시 |
