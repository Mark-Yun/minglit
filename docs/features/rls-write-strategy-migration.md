...
> 아래 목록은 **Flutter 클라이언트(minglit_kit)에서 직접 `.insert()/.update()/.delete()`를 호출**하는 테이블만 포함합니다.
> Worker/트리거 경유 write는 별도 섹션 참고.
...
### Worker/트리거 경유 write 테이블 (클라이언트 직접 write 없음)

> 아래 테이블은 RLS write 정책이 있지만, 클라이언트가 직접 write하지 않고 **Edge Function 또는 DB 트리거**를 통해서만 write됩니다.

| 테이블 | 경유 | RLS write 정책 | 비고 |
|--------|------|---------------|------|
| `user_profiles` | identity-verify, profile-update EF | Users can update own profile (UPDATE) | 클라이언트에서 직접 update 호출 없음 |

### RLS 위반 정책

#### Frontend 직접 write (EF 전환 대상)
...
#### Worker 경유 write (현재 안전, 전환 불필요)

| 테이블 | 정책 | 경유 |
|--------|------|------|
| `user_profiles` | Users can update own profile | identity-verify, profile-update EF |
...
| `partner-manage-verification` | verifications | verification_command_repository |
| `partner-review-submission` | verification_submissions (심사+코멘트) | verification_command_repository |
...
| `partner-register` | partner_applications | partner_application_repository |
...
- `user_profiles`는 이미 Worker 경유 처리이므로 전환 불필요 (RLS write 정책은 향후 제거 검토)
