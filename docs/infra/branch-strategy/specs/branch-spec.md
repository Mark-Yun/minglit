...
| `main` | 영구 | yes (가장 강함) |
| `feat/*`, `fix/*`, `chore/*`, `docs/*`, `dev/hotfix/*`, `rc/hotfix/*`, `main/hotfix/*` | 임시 (PR 머지 후 자동 삭제) | no |
| `cherry-pick/*` | 자동 생성 (`rc-hotfix-apply`, main hotfix follow-up) | no |
...
> dev 는 기본적으로 사람이 직접 PR 안 만듦. `dev-staging-dev-cut` workflow 가 만든 `cut/dev-staging-dev/*` 만 통과한다. 예외적으로 [hotfix-policy.md](../hotfix-policy.md) 의 `dev/hotfix/*` + `hotfix:dev-approved` 조건을 만족하면 통과한다.
...
> `dev-rc-cut` workflow 가 branch 생성 후 이 protection ruleset 을 REST API 로 활성화. RC 는 기본적으로 dev-staging 에 먼저 머지된 fix commit/snapshot 의 approved `rc/hotfix/*` cherry-pick PR 만 받는다. RC-only 선머지는 release-manager override 가 필요한 예외다. RC 종료 후 `minglit-release-bot` 이 branch 를 삭제하고, 릴리즈 이력은 `promo/rc-*`, `promo/main-*`, GitHub Release asset 으로 보존한다.
...
| 컨벤션 (일부 gate 강제) | 일반 작업은 `dev-staging` base. Hotfix 는 대상 base prefix 를 먼저 둔다 (`dev/hotfix/*`, `rc/hotfix/*`, `main/hotfix/*`) |
...
|------|----|
| Protection | none |
| 생성 방식 | `rc-hotfix-apply` 또는 main hotfix follow-up workflow 가 자동 생성 |
...
| `auto-delete-temp` | `feat/**`, `fix/**`, `chore/**`, `docs/**`, `dev/hotfix/**`, `rc/hotfix/**`, `main/hotfix/**`, `cherry-pick/**` | auto-delete only |
...
| `cherry-pick/**` | `rc-hotfix-apply` / main hotfix follow-up workflow only |
