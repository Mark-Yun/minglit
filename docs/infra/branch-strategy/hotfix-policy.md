...
`dev`, `rc/*`, `main` 은 일반 작업 PR 을 직접 받지 않는다. 정상 진입점은 항상 `dev-staging` 이다. Hotfix 는 예외 흐름이며, branch name + PR label + linked issue label 로 명시 승인된 경우에만 branch별 PR gate 를 통과한다. RC hotfix 의 기본 source-of-truth 도 dev-staging 이며, RC 에는 dev-staging 에 머지된 fix commit/snapshot 을 cherry-pick/promote 한다.
...
| `rc/*` | dev-staging 에 먼저 머지된 fix commit 의 cherry-pick PR | `rc/hotfix/*` + `hotfix:rc-approved` on PR and linked issue. RC-only 선머지는 release-manager override 필요 |
...
RC hotfix 는 먼저 `dev-staging` 에 머지하고, active RC 에는 같은 fix commit/snapshot 을 cherry-pick 한다. 이렇게 하면 다음 RC 후보가 dev-staging/dev 흐름을 타면서 자연스럽게 fix 를 포함하므로, per-hotfix backport issue 를 gate SSOT 로 추적하지 않는다.

| 상황 | 동작 |
|------|------|
| RC soak 중 blocker 발견 | fix PR → `dev-staging` merge → `rc-hotfix-apply` 가 active RC cherry-pick PR 생성 |
| RC-only 선머지 override | `rc/hotfix/*` → `rc/YYYY-Wxx` merge 후 release-manager 가 follow-up dev-staging PR 을 추적 |
| main hotfix | `main-hotfix-backport` 또는 수동 release-manager PR 로 `dev-staging` + 필요 시 active `rc/*` 에 반영 |

Cherry-pick/follow-up 실패는 `P1-high` 이상 issue 로 기록한다. main hotfix follow-up 실패는 `P0-critical` 로 승격할 수 있다.
