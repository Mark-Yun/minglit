...
void main() {
  ...
  group('Event Application RLS Tests', () {
    ...
    setUpAll(() async {
      ...
      final event = await adminClient.from('events').select('id, tickets(id)').limit(1).single();
      ...
      await adminClient.from('event_participants').delete().inFilter('user_id', [user1Id, user2Id]);
      await adminClient.from('event_applications').delete().inFilter('user_id', [user1Id, user2Id]);

      final res1 = await adminClient.from('event_applications').insert({
        'event_id': eventId,
        'ticket_id': ticketId,
        'user_id': user1Id,
        'status': 'pending',
      ...
      final res2 = await adminClient.from('event_applications').insert({
        'event_id': eventId,
        'ticket_id': ticketId,
        'user_id': user2Id,
        'status': 'pending',
    ...
    test('User 1 should see their own application', () async {
      ...
      final data = await client1.from('event_applications').select().eq('id', app1Id).maybeSingle();
    ...
    test('User 1 should NOT see User 2\'s application', () async {
      final client1 = createUserClient(user1Id);
      final data = await client1.from('event_applications').select().eq('id', app2Id).maybeSingle();
    ...
    test('User 1 should NOT be able to update User 2\'s application', () async {
      ...
      expect(res, isEmpty, reason: 'RLS should prevent updating other users data');
      
      // Verify via admin that it remains pending
      final verify = await adminClient.from('event_applications').select('status').eq('id', app2Id).single();
      expect(verify['status'], equals('pending'));
    });

    test('User 1 should be able to update their own application (message)', () async {
      final client1 = createUserClient(user1Id);
      
      await client1
          .from('event_applications')
          .update({'message': 'Hello from User 1'})
          .eq('id', app1Id);
      
      final verify = await adminClient.from('event_applications').select('message').eq('id', app1Id).single();
