...
void main() {
  ...
  group('Refund Trigger Tests', () {
    ...
    test('Rejection Cascade: Submission Rejected -> App Rejected + Reason + Refund Requested', () async {
      ...
      await adminClient.from('verification_submissions').update({
        'status': 'rejected',
      ...
      // A. Status Rejected?
      expect(updatedApp['status'], equals('rejected'), reason: 'Application should be rejected');
      
      // B. Reason Synced?
      expect(updatedApp['rejection_reason'], equals(rejectionReason), reason: 'Rejection reason should be synced');
      ...
      expect(updatedApp['refund_status'], equals('requested'), reason: 'Refund status should be requested');
