...
void main() {
  ...
  group('Auto-Approval Trigger Logic Tests', () {
    ...
    test('Chain Reaction: Verify Approved -> Application Approved -> Ticket Issued', () async {
      ...
      final appRes = await adminClient.from('event_applications').insert({
      ...
      final subRes = await adminClient.from('verification_submissions').insert({
      ...
      await adminClient.from('verification_submissions').update({'status': 'approved'}).eq('id', subId);
      ...
      final verifiedUser = await adminClient
          ...
          .maybeSingle();
      expect(verifiedUser, isNotNull, reason: 'partner_verified_users should be created via trigger');

      // B. event_applications status changed?
      final updatedApp = await adminClient.from('event_applications').select('status').eq('id', appId).single();
      expect(updatedApp['status'], equals('approved'), reason: 'Application should be auto-approved');
      ...
      final participant = await adminClient
          ...
          .maybeSingle();
      expect(participant, isNotNull, reason: 'Ticket should be issued automatically');
      expect(participant!['ticket_code'], isNotNull);
      
      print('🎉 Chain reaction verified successfully!');
    });

    test('Revoke: Changing submission to rejected should remove verified_user', () async {
      ...
      await adminClient.from('verification_submissions').update({'status': 'rejected'}).eq('id', subId);
      ...
      final verifiedUser = await adminClient
          ...
          .eq('submission_id', subId)
          .maybeSingle();
      expect(verifiedUser, isNull, reason: 'partner_verified_users should be removed on revoke');
