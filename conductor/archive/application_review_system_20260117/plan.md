...
- [x] Task: Conductor - User Manual Verification 'Phase 1: 환불 로직 테스트' (Protocol in workflow.md)
...
- [x] Task: Conductor - User Manual Verification 'Phase 2: 심사 기능 확인' (Protocol in workflow.md)

## Phase 3: 유저 알림 및 재신청 유도 (`app_user`)
- [x] Task: 심사 결과 알림 핸들링 2487ed9
    - [x] 승인/거절 푸시 알림 수신 시 딥링크 처리 확인.
- [x] Task: 재신청 플로우 확인 2487ed9
    - [x] 거절된 유저가 파티 화면 진입 시 "신청하기" 버튼 활성화 여부 확인 (기존 `EventAdmissionController` 로직 재검증).
- [x] Task: Conductor - User Manual Verification 'Phase 3: E2E 시나리오 테스트' (Protocol in workflow.md)
