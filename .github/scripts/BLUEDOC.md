...
| [`check-bluedoc-freshness.sh`](./check-bluedoc-freshness.sh) | PR 에서 새/삭제 파일에 대해 가장 가까운 ancestor BLUEDOC.md 갱신 강제. `EXEMPT_FRESH_BLUEDOC=<reason>` commit message 로 예외 처리 | `pr-gate.check-bluedoc-freshness` (required check) |
