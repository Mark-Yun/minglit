...
Deno.test({
  name: "user-create-order - happy path: paid event creates order",
  fn: async () => {
...
Deno.test({
  name:
    "user-create-order - happy path: free event creates order with requires_payment=false",
  fn: async () => {
    await withEnv(ENV, async () => {
      const handler = await captureServeHandler(
        new URL("./index.ts", import.meta.url),
...
Deno.test({
  name: "user-create-order - unauthenticated request returns 401",
  fn: async () => {
    await withEnv(ENV, async () => {
      const handler = await captureServeHandler(
        new URL("./index.ts", import.meta.url),
...
Deno.test({
  name: "user-create-order - missing event_id returns 400",
  fn: async () => {
    await withEnv(ENV, async () => {
      const handler = await captureServeHandler(
        new URL("./index.ts", import.meta.url),
...
Deno.test({
  name: "user-create-order - missing ticket_id returns 400",
  fn: async () => {
    await withEnv(ENV, async () => {
      const handler = await captureServeHandler(
        new URL("./index.ts", import.meta.url),
...
Deno.test({
  name: "user-create-order - malformed JSON returns 400",
  fn: async () => {
    await withEnv(ENV, async () => {
      const handler = await captureServeHandler(
        new URL("./index.ts", import.meta.url),
...
Deno.test({
  name: "user-create-order - nonexistent event returns 404",
  fn: async () => {
    await withEnv(ENV, async () => {
      const handler = await captureServeHandler(
        new URL("./index.ts", import.meta.url),
      ...
      const { fetchMock } = createFetchMock([
        authRoute,
        {
          ...
          handler: () =>
            jsonResponse({ message: "not found" }, { status: 406 }),
        },
      ]);
...
Deno.test({
  name: "user-create-order - cancelled event returns EVENT_CLOSED",
  fn: async () => {
    await withEnv(ENV, async () => {
      const handler = await captureServeHandler(
        new URL("./index.ts", import.meta.url),
...
Deno.test({
  name: "user-create-order - past event returns EVENT_NOT_SCHEDULED",
  fn: async () => {
    await withEnv(ENV, async () => {
      const handler = await captureServeHandler(
        new URL("./index.ts", import.meta.url),
...
Deno.test({
  name: "user-create-order - ticket not belonging to event returns 404",
  fn: async () => {
    await withEnv(ENV, async () => {
      const handler = await captureServeHandler(
        new URL("./index.ts", import.meta.url),
...
Deno.test({
  name: "user-create-order - sold out ticket returns TICKET_SOLD_OUT",
  fn: async () => {
    await withEnv(ENV, async () => {
      const handler = await captureServeHandler(
        new URL("./index.ts", import.meta.url),
...
Deno.test({
  name: "user-create-order - full event returns EVENT_FULL",
  fn: async () => {
    await withEnv(ENV, async () => {
      const handler = await captureServeHandler(
        new URL("./index.ts", import.meta.url),
...
Deno.test({
  name: "user-create-order - unverified user returns IDENTITY_REQUIRED",
  fn: async () => {
    await withEnv(ENV, async () => {
      const handler = await captureServeHandler(
        new URL("./index.ts", import.meta.url),
...
Deno.test({
  name: "user-create-order - gender mismatch returns GENDER_MISMATCH",
  fn: async () => {
    await withEnv(ENV, async () => {
      const handler = await captureServeHandler(
        new URL("./index.ts", import.meta.url),
...
Deno.test({
  name: "user-create-order - age mismatch returns AGE_MISMATCH",
  fn: async () => {
    await withEnv(ENV, async () => {
      const handler = await captureServeHandler(
        new URL("./index.ts", import.meta.url),
...
Deno.test({
  name: "user-create-order - balance limit returns BALANCE_LIMIT",
  fn: async () => {
    await withEnv(ENV, async () => {
      const handler = await captureServeHandler(
        new URL("./index.ts", import.meta.url),
...
Deno.test({
  name: "user-create-order - duplicate application returns ALREADY_APPLIED",
  fn: async () => {
    await withEnv(ENV, async () => {
      const handler = await captureServeHandler(
        new URL("./index.ts", import.meta.url),
      ...
      const { fetchMock } = createFetchMock([
        ...
        restRoute("entry_groups", "GET", [MOCK_ENTRY_GROUP]),
        rpcRoute("check_party_balance", MOCK_BALANCE_OK),
        restRoute("event_applications", "GET", {
          id: "existing-app",
          status: "approved",
        }),
      ]);
...
Deno.test({
  name:
    "user-create-order - missing verification data returns VERIFICATION_REQUIRED",
  fn: async () => {
    await withEnv(ENV, async () => {
      const handler = await captureServeHandler(
        new URL("./index.ts", import.meta.url),
