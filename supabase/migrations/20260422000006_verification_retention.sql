...
-- ── 0. db_custom_fn 열거형 확장 (PR #1705와 병행 가능 — IF NOT EXISTS 사용) ─────
...
INSERT INTO admin.retention_policies (
  ...
  true,
  '개인정보처리방침 §F6: 자격 인증 제출 기록 PII 1년 후 익명화 (snapshot_data→{}, admin_comment/reviewed_by→NULL)',
  jsonb_build_object(
    'approach', 'anonymize',
    'reason',   'partner_verified_users.submission_id ON DELETE CASCADE — 하드 DELETE 시 활성 인증 박탈'
  )
);
