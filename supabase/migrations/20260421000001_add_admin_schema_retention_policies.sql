-- 1. admin schema + default privileges
...
-- 2. ENUM
CREATE TYPE admin.retention_kind AS ENUM ('db_table', 'storage_bucket', 'pgmq_archive');

-- 3. retention_policies table
...
CREATE INDEX retention_policies_enabled_idx ON admin.retention_policies(enabled) WHERE enabled;
CREATE INDEX retention_policies_kind_idx ON admin.retention_policies(kind);
...
-- 6. Helper RPC — delete old rows
CREATE OR REPLACE FUNCTION admin.delete_old_rows(
  p_schema text, p_table text, p_ts_col text, p_cutoff_days int
) RETURNS bigint LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE
  deleted_count bigint;
BEGIN
  -- Safety guard: schema whitelist (SQL injection 방어)
...
-- 7. Helper RPC — archive old pgmq messages
CREATE OR REPLACE FUNCTION admin.archive_old_pgmq_messages(
  p_queue_name text, p_cutoff_days int
) RETURNS bigint LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE
...
-- 8. Main default seed (both envs; dev is overridden via seed.sql dev section)
INSERT INTO admin.retention_policies (id, kind, retention_days, target, description) VALUES
('cron_job_run_details', 'db_table', 30,
 '{"schema":"cron","table":"job_run_details","ts_col":"end_time"}'::jsonb,
 'pg_cron 실행 이력'),
('net_http_response', 'db_table', 7,
 '{"schema":"net","table":"_http_response","ts_col":"created"}'::jsonb,
 'net.http_post 응답 로그'),
('pgmq_global_events_archive', 'pgmq_archive', 14,
 '{"queue_name":"q_global_events"}'::jsonb,
 'global_events 큐 archive retention'),
('pgmq_vectors_archive', 'pgmq_archive', 14,
 '{"queue_name":"q_vectors"}'::jsonb,
 'vectors 큐 archive retention (pgvector 비동기 처리)'),
('pgmq_notifications_archive', 'pgmq_archive', 7,
 '{"queue_name":"q_notifications"}'::jsonb,
 'notifications 큐 archive retention'),
('storage_bug_reports', 'storage_bucket', 30,
 '{"bucket_id":"bug-report-attachments"}'::jsonb,
 'QA 버그 리포트 첨부 파일 — 현재 390MB 점유, 짧게 시작 후 Admin UI 로 조정')
