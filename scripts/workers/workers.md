...
| 워커 | 스케줄 | 역할 |
|------|--------|------|
| **tpm-staff** | 10분마다 | 안을 봄 — audit→actionable 이슈 + 이슈 품질 관리 + 운영 분석 + report-exec |
...
| 워커 | 스케줄 | 역할 |
|------|--------|------|
| **issue-worker** | 10분마다 | needs-dev만 처리 + PR 케어 (BEHIND 업데이트, dependabot, 리뷰 대응) |
...
| `needs-tpm` | 모든 워커 | tpm-staff |
| `needs-dev` | tpm-staff | issue-worker |
| `bug-report` | 유저/앱 | tpm-staff |
| `bug` | 수동/tpm | tpm-staff |
| `ci-failure` | CI notify | tpm-staff |
| `enhancement` | 수동/tpm | tpm-staff |
| `refactor` | 수동/tpm | tpm-staff |
