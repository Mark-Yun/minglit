#!/bin/bash
# audit-security-worker.sh — git worktree 격리 + 24시간 단위 감사
...
while true; do
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Launching audit-security worker..."
    ...
    /usr/local/bin/claude -p "$(cat "$PROMPT_FILE")" \
        ...
        2>&1 | tee "/tmp/claude-audit-security-$(date +%Y%m%d-%H%M%S).log"
