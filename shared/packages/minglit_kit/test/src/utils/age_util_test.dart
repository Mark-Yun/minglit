...
void main() {
  ...
  group('AgeUtil', () {
    ...
    group('calculateManAge', () {
      ...
      test('Korean age is always 1 more than Man age (year-only approximation)',
          () {
    ...
    group('calculateManAgeFromDate', () {
      ...
      test('birthday yesterday → age is exact (birthday passed)', () {
        ...
        final birthDate =
            DateTime(now.year - 30, yesterday.month, yesterday.day);
        expect(AgeUtil.calculateManAgeFromDate(birthDate), 30);
      });
      ...
      test('born Nov 15 1995: correct 만 age based on whether today is before or after Nov 15',
          () {
        ...
        final now = DateTime.now();
        final birthdayPassed = (now.month > 11) ||
            (now.month == 11 && now.day >= 15);
        final expected =
            birthdayPassed ? now.year - 1995 : now.year - 1995 - 1;
