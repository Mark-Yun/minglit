...
mixin _TagRepositoryQueries on _SupabaseTagContext {
  ...
  Future<List<Event>> getPartiesByTag(
    ...
    try {
      final data = await supabaseClient.rpc<dynamic>(
        'get_parties_by_tag',
        params: {'p_tag_id': tagId, 'p_limit': limit, 'p_offset': offset},
      ) as List;
      return data
  ...
  Future<List<Tag>> getUserInterestTags() async {
    try {
      final data = await supabaseClient
          ...
          .select('tag_id, tags(*)')
          .eq('user_id', supabaseClient.auth.currentUser!.id) as List;
      return data
          .map(
            (row) => Tag.fromJson(
